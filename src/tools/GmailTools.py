...
class EmailInfoExtractor:
    def __init__(self):
        ...
        You are a contract assistant that extracts structured information from an email.
        The user will provide the raw email body.
        Return your answer strictly as JSON with the following fields:

        {{
            "contract_type": string,   # e.g. "NDA", "Service Agreement", "Vendor Contract"
            "party_1": string,         # e.g. sender or company name
            "party_2": string,         # e.g. recipient company
            "validity_period": string, # e.g. "2 years", "6 months"
            "purpose": string,         # e.g. "project collaboration", "data sharing"
            "key_terms": string        # brief summary of main ask
        }}
    ...
    def extract(self, email_body: str) -> dict:
        try:
            response = self.llm.invoke(self.prompt.format(email_body=email_body))
            ...
            print(f"⚠️ Error parsing contract info: {e}")
            return {}
...
class GmailToolsClass:
    ...
    def _get_email_info(self, msg_id):
        ...
        # 🔹 Extract structured contract fields using Gemini
        # 🔹 Extract structured contract fields using Gemini
        try:
            print(f"🔍 Extracting contract details for email '{email_info['subject']}'...")
            extractor = EmailInfoExtractor()  # ✅ create instance of your Gemini helper
            parsed_data = extractor.extract(body)  # ✅ use its extract() method

            if parsed_data:
                email_info.update(parsed_data)
                print("✅ Extracted contract metadata:", parsed_data)
            else:
                print("⚠️ No structured JSON detected in Gemini response.")
        except Exception as e:
            print(f"⚠️ Error extracting structured data: {e}")


        # 🔹 Save enriched email to MongoDB
        try:
            emails_collection.insert_one(email_info)
            print("💾 Email with extracted contract info saved to MongoDB.")
        except Exception as db_err:
            print(f"⚠️ MongoDB insert failed: {db_err}")
