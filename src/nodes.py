...
import json
import time
from .contract_rag_agent import ContractRAGAgent
...
class Nodes:
    def __init__(self):
        self.agents = Agents()
        self.gmail_tools = GmailToolsClass()
        self.contract_agent = ContractRAGAgent()
    ...
    def route_email_based_on_category(self, state: GraphState) -> str:
        """Routes the email based on its category."""
        print(Fore.YELLOW + "Routing email based on category...\n" + Style.RESET_ALL)
        category = state["email_category"].strip().lower()

        if category in ["legal_contractdraftrequest", "contract draft request"]:
            print(Fore.GREEN + "📄 Routing to ContractRAGAgent for contract generation..." + Style.RESET_ALL)
            return "contract draft request"  # matches the key in graph.py → generate_contract_from_email
        elif category in ["contract_review", "contract review"]:
            return "contract review"
        elif category in ["legal_query", "legal query"]:
            return "legal query"
        elif category == "unrelated":
            return "unrelated"
        else:
    ...
    def generate_contract_from_email(self, state: GraphState) -> GraphState:
        """Generates a full contract draft using ContractRAGAgent and Pinecone templates."""
        print("🧾 Generating contract draft using ContractRAGAgent...\n")
        current_email = state["current_email"]

        try:
            # Call the RAG agent to generate contract text
            contract_draft = self.contract_agent.generate_contract(
                                                    current_email.body,
                                                    {
            ...
            contract_draft = "Error generating contract draft."

        # Store the generated contract draft in the state so writer can include it in the reply
        return {
            "generated_contract": contract_draft,
            "retrieved_documents": "The draft has been created and this is the confirmation email regarding the same"
            # "retrieved_documents": contract_draft  # reuse for email writer context
        }
